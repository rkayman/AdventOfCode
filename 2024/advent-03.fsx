...
let loggerAgent (token: CancellationToken option) =
    let cancel = token |> Option.defaultValue Async.DefaultCancellationToken
...
let logger : Agent<string> = loggerAgent None
...
type MulEvent =
    ...
    | Noop
    | Reset
    | Result of AsyncReplyChannel<int>

let mulAgent (token: CancellationToken option) =
    let cancel = token |> Option.defaultValue Async.DefaultCancellationToken
    Agent.Start(fun agent ->
        let rec enabled total = async {
            if cancel.IsCancellationRequested then
                agent.Dispose()
            let! msg = agent.Receive()
            match msg with
            | Mul (x, y) ->
                let result = x * y
                printfn $"[MUL] ({x},{y}) = {result}, total = {total + result}"
                return! enabled (total + result)
            | Don't -> 
                printfn $"[INFO] disabled...{total}"
        ...
        and disabled total = async {
            if cancel.IsCancellationRequested then
                agent.Dispose()
            let! msg = agent.Receive()
            match msg with
            | Do ->
                printfn $"[INFO] enabled...{total}"
                return! enabled total
            | Reset ->
                printfn "[INFO] reset..."
                return! enabled 0
...
let mul : Agent<MulEvent> = mulAgent None
...
// Define the parser for "mul(X,Y)"
let mulParser =
    let mulStart = pstring "mul("
    let mulEnd = pchar ')'
    let comma = pchar ','
    let number = pint32    
    mulStart >>. number .>> comma .>>. number .>> mulEnd |>> (fun (x, y) -> Mul (x, y))

let doParser =
    let doMatch = pstring "do()"
    doMatch |>> (fun _ -> Do)

let dontParser =
    let dontMatch = pstring "don't()"
    dontMatch |>> (fun _ -> Don't)
    
// Define a parser that finds all occurrences of the pattern and skips corrupted text
let allMulParser =
    let choices = choice [mulParser; doParser; dontParser]
    let noop = skipAnyChar |>> (fun _ -> Noop)
    manyTill (attempt choices <|> noop) eof

// Function to parse the input string and calculate the sum
let calculateMulSum input =
    match run allMulParser input with
    | Failure (errorMsg, _, _) ->
        failwith errorMsg
    | Success (results, _, _) ->
        results
        |> List.iter mul.Post
        mul.PostAndReply Result 
...
let input = "mul(2,4) some text mul(3,7) more text mul(5,5)"
mul.Post Reset 
let result = calculateMulSum given 
printfn $"The sum of all multiplications is: %d{result}"